/*
 *
 * GamesPage constants
 *
 */

const page = 'app/GamesPage/';
export const GET_RESULT = `${page}GET_RESULT`;
export const GET_RESULT_SUCCESS = `${page}GET_RESULT_SUCCESS`;
export const GET_RESULT_FAILED = `${page}GET_RESULT_FAILED`;
export const GET_GAME_INFO = `${page}GET_GAME_INFO`;
export const GET_GAME_INFO_SUCCESS = `${page}GET_GAME_INFO_SUCCESS`;
export const GET_GAME_INFO_FAILED = `${page}GET_GAME_INFO_FAILED`;
export const GET_GAME_TOKEN = `${page}GET_GAME_TOKEN`;
export const GET_GAME_TOKEN_SUCCESS = `${page}GET_GAME_TOKEN_SUCCESS`;
export const GET_GAME_TOKEN_FAILED = `${page}GET_GAME_TOKEN_FAILED`;
export const GET_MEMBER_INFO = `${page}GET_MEMBER_INFO`;
export const GET_MEMBER_INFO_SUCCESS = `${page}GET_MEMBER_INFO_SUCCESS`;
export const GET_MEMBER_INFO_FAILED = `${page}GET_MEMBER_INFO_FAILED`;
